...
export async function sendWhatsAppMessage(input) {
    ...
    const accountSid = requireConfig(disasterConfig.whatsapp.twilioAccountSid, 'Set TWILIO_ACCOUNT_SID to enable WhatsApp sending.');
    const authToken = requireConfig(disasterConfig.whatsapp.twilioAuthToken, 'Set TWILIO_AUTH_TOKEN to enable WhatsApp sending.');
    const from = requireConfig(disasterConfig.whatsapp.twilioWhatsAppFrom, 'Set TWILIO_WHATSAPP_FROM to enable WhatsApp sending.');
...
function buildExpectedTwilioSignature(authToken, url, params) {
...
export function validateTwilioWebhookSignature(input) {
    const authToken = requireConfig(disasterConfig.whatsapp.twilioAuthToken, 'Set TWILIO_AUTH_TOKEN to validate incoming Twilio webhook requests.');
    if (!input.signature)
        return false;
