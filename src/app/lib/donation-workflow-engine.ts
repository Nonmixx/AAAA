...
function inferMissingFields(userLatest: string, categorySlug: string): string[] {
  ...
  const hasCondition =
    /\b(new|gently\s*used|lightly\s*worn|unworn|tags?\s*on|excellent|good\s*cond|fair|worn|damaged|like\s*new)\b/i.test(ul);
...
function buildLogistics(agents: PlanReceiver[], userLatest: string): DonationLogisticsAssessment {
  ...
  const summary = hasDeliveryPref
    ? 'Donor hinted at logistics — only platform-funded pickup (Corporate Logistics Wallet or Community Crowdfunding Board) or self drop-off; never donor-paid couriers.'
    : ngo_dropoff_near_catalog
      ? 'Partners exist within catalog distances — reassure: self drop-off or platform-funded driver (two rails); donor never pays Lalamove/Grab.'
...
function defaultDeliveryOptions(): DonationDeliveryOption[] {
  return [
    {
      id: 'pickup_corporate_wallet',
      label: 'Platform-funded pickup — Corporate Logistics Wallet',
      description:
        'DonateAI uses the corporate wallet to fund a driver to collect from you. The donor never books or pays third-party couriers.',
      recommended: true,
    },
    {
      id: 'pickup_community_board',
      label: 'Platform-funded pickup — Community Crowdfunding Board',
      description:
        'Community crowdfunding can fund a driver for eligible routes. Still no Lalamove/Grab/courier cost to the donor.',
    },
    {
      id: 'self_dropoff',
...
function buildQuestions(missing: string[], categoryDisplay: string): string[] {
  ...
  if (missing.includes('condition'))
    qs.push('What condition are they in overall? (e.g. gently used, like new — we also confirm from a photo after Send.)');
  if (!qs.length) qs.push('Confirm details or attach a clear item photo and press Send when ready.');
  return qs.slice(0, 4);
}
...
export function buildDonationWorkflowSnapshot(p: BuildWorkflowParams): DonationWorkflowSnapshot {
  ...
  const next_step_user =
    intent === 'unsupported'
      ...
      : stage === 'awaiting_image'
        ? 'Attach a clear photo and press Send to run condition screening.'
        : missing_fields.length > 0
          ? 'Answer the open questions (category detail, quantity, condition, location), then continue.'
          : 'Add a photo when ready and press Send.';
