...
import { geocodeAddress, getTravelEstimate, type LatLng } from './providers/maps';
...
export async function ingestExternalSignals(supabase: SupabaseClient) {
  ...
  if (newsSignals.status === 'fulfilled') {
    for (const signal of newsSignals.value) {
      created.push(
        await createIncidentSignal(supabase, {
          sourceType: 'news',
...
export async function sendShelterOutreach(supabase: SupabaseClient, disasterEventId: string) {
  ...
  for (const contact of data ?? []) {
    ...
    await supabase.from('communications').insert({
      ...
      transcript: body,
      provider_message_id:
        typeof providerResponse === 'object' && providerResponse && 'sid' in providerResponse
...
export async function previewShelterOutreach(supabase: SupabaseClient, disasterEventId: string) {
  const { data, error } = await supabase
    .from('shelter_contacts')
    .select('id, organization_id, contact_name, phone, is_primary')
    .eq('is_verified', true)
...
export async function recomputeEquityMetrics(supabase: SupabaseClient, disasterEventId: string) {
  ...
  const grouped = new Map<string, { organizationId: string; category: string; requested: number; fulfilled: number; emergency: boolean }>();
...
export async function getActiveDisasterEventId(supabase: SupabaseClient) {
...
async function resolvePointLocation(address: string | null, latitude: unknown, longitude: unknown): Promise<LatLng | null> {
...
export async function createRoutePlan(supabase: SupabaseClient, options: RoutePlanningOptions) {
  ...
  if (collectionPointError) throw collectionPointError;
  ...
  const collectionPointLocation = await resolvePointLocation(
    collectionPoint.address ?? null,
  ...
  if (!allocations?.length) {
    const [{ data: scheduledAllocations, error: scheduledAllocationsError }, { data: existingRoutePlans, error: existingRoutePlansError }] =
      await Promise.all([
        supabase
          .from('donation_allocations')
          .select('id, need_id, allocated_quantity, status, delivery_method, scheduled_at, route_summary, routing_notes')
          .in('need_id', needIds)
          .eq('delivery_method', 'platform_delivery')
    ...
    if (scheduledAllocationsError) throw scheduledAllocationsError;
    if (existingRoutePlansError) throw existingRoutePlansError;

    if (scheduledAllocations?.length) {
      return {
        routePlan: existingRoutePlans?.[0] ?? null,
        routePlans: existingRoutePlans ?? [],
        stops: [],
        skippedAllocations: [],
        reason: 'Platform deliveries have already been routed and scheduled for this disaster event.',
        alreadyPlanned: true,
      ...
      need.id,
      {
        ...
        organizationName:
          typeof need.organizations === 'object' && need.organizations && 'name' in need.organizations
            ? String((need.organizations as Record<string, unknown>).name ?? 'Organization')
            : 'Organization',
        organizationAddress:
          typeof need.organizations === 'object' && need.organizations && 'address' in need.organizations
            ? ((need.organizations as Record<string, unknown>).address as string | null)
            : null,
        organizationLatitude:
          typeof need.organizations === 'object' && need.organizations && 'latitude' in need.organizations
            ? (need.organizations as Record<string, unknown>).latitude
            : null,
        organizationLongitude:
          typeof need.organizations === 'object' && need.organizations && 'longitude' in need.organizations
            ? (need.organizations as Record<string, unknown>).longitude
            : null,
      },
    ]),
  );
  ...
  const skippedAllocations: Array<{ allocationId: string; reason: string }> = [];
  ...
  for (const allocation of allocations) {
    ...
    const existing: RouteDestination = grouped.get(key) ?? {
  ...
  const plannedStops = await Promise.all(
  ...
  for (const [index, stop] of plannedStops.entries()) {
    ...
    await supabase.from('route_allocations').insert(
      stop.destination.allocations.map((allocation) => ({
        route_plan_id: routePlan.id,
    ...
    const routeSummary = stop.estimate
      ? `Stop ${index + 1}: ${stop.destination.organizationName} (~${Math.round(stop.estimate.durationSeconds / 60)} min, ${(
          stop.estimate.distanceMeters / 1000
        ).toFixed(1)} km from ${collectionPoint.name}).`
    ...
    await supabase
      ...
      .in(
        'id',
        stop.destination.allocations.map((allocation) => allocation.allocationId),
      );
  }
